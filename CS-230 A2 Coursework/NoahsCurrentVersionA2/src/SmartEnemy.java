...
public class SmartEnemy extends Body {
	...
	private int playX = player.getX();
	private int playY = player.getY();
...
//	private int d ;//to state the unwalkable object is on the up or the down side of the enemy when in same x
	...
	public void move() {
		if (this.playX == this.x) {
			
		}
		if (this.playY == this.y) {
...
//			if ((objectList.get(k).getX() == this.c) && objectList.get(k).getY() == this.y+1){//this.c make us can change form x+1 or x-1
...
//			if ((objectList.get(t).getX() == this.c) && objectList.get(t).getY() == this.y-1){//this.c make us can change form x+1 or x-1 just y change form ++ to -- cuz is under
...
//			if  ((objectList.get(r).getX() == this.x-1) && objectList.get(r).getY() == this.d) {//this.d so we can change form y-1 or y+1
...
//			if  ((objectList.get(p).getX() == this.x+1) && objectList.get(p).getY() == this.d) {
